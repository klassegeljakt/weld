...
impl Token {
    ...
    pub fn signals_type(&self) -> bool {
        use self::Token::*;
        match *self {
            TI8 | TI16 | TI32 | TI64 | TU8 | TU16 | TU32 | TU64 | TF32 | TF64 | TBool | TVec
            | TSimd | TAppender | TMerger | TDict | TDictMerger | TGroupMerger | TVecMerger
            | TOpenBrace | TQuestion => true,
            _ => false,
        }
    }
}
...
fn basic_tokenize() {
    ...
    assert_eq!(
        tokenize("\"test string\"").unwrap(),
        vec![TStringLiteral("test string".to_string()), TEndOfInput]
    );
    assert_eq!(
        tokenize("\"test\" string").unwrap(),
        vec![
            TStringLiteral("test".to_string()),
            TIdent("string".into()),
            TEndOfInput
        ]
    );
    assert_eq!(
        tokenize("a for 23 + z0").unwrap(),
        vec![
            TIdent("a".into()),
            TFor,
            TI32Literal(23),
    ...
    assert_eq!(
        tokenize("|a:vec[i8]| slice(a, 2L, 3L)").unwrap(),
        vec![
            TBar,
            TIdent("a".into()),
            TColon,
    ...
    assert_eq!(
        tokenize("iffy if").unwrap(),
        vec![TIdent("iffy".into()), TIf, TEndOfInput]
    );

    assert_eq!(
        tokenize("keyexists(a, 1)").unwrap(),
        vec![
            TKeyExists,
            TOpenParen,
            TIdent("a".into()),
    ...
    assert_eq!(
        tokenize("0x10").unwrap(),
        vec![TI32Literal(16), TEndOfInput]
    );

    assert_eq!(
        tokenize("1e-5f").unwrap(),
    ...
    assert_eq!(
        tokenize("dictmerger[i32,i32,+] @[]").unwrap(),
        vec![
            TDictMerger,
            TOpenBracket,
            TI32,
    ...
    assert_eq!(
        tokenize("sort(a, |x:i32| x)").unwrap(),
        vec![
            TSort,
            TOpenParen,
            TIdent("a".into()),
